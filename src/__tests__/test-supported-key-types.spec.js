...
const testJwkDeprecated = async privateKeyJwk => {
  const key = new JsonWebKeyLinkedDataKeyClass2020({
    id: "did:example:123#" + privateKeyJwk.kid,
...
describe("test supported key types", () => {
  ...
  it("should be able to create a verify with deprecated keyTypes ", async () => {
    await Promise.all(didDocJwks.keys.map(testJwkDeprecated));
  });
});
